...
class kps_reporting
{
    ...
    {
        // Email vorbereiten
        $reportSubject = esc_html(__('Eintrag gemeldet', 'kps')) . ': ' . get_bloginfo('name');;

        if ($this->_sendReportAuto === true)
        {
            $reportContent = esc_html(__('Es wurde ein Eintrag durch die automatische Sperre gesperrt!', 'kps'));
        }
        elseif ($this->_sendReportComplet === true)
        {
            $reportContent = esc_html(__('Es wurde die Gesamtmenge an Meldungen für den Eintrag erreicht!', 'kps'));
        }
        else
        {
            $reportContent = esc_html(__('Es wurde ein Eintrag gemeldet!', 'kps'));
        }

        $reportContent .= '

' . esc_html(__('Grund', 'kps')) . ': ' . $this->_reportReason . '
' . esc_html(__('Zeitstempel', 'kps')) . ': ' . date_i18n(get_option('date_format'), current_time('timestamp')) .' @ '. date_i18n(get_option('time_format'), current_time('timestamp')) . '

' . esc_html(__('Bearbeitenungs-Link', 'kps')) . ':
**************************
' . KPS_ADMIN_URL . '/entries.php&edit_id=' . $id . '

' . esc_html(__('Systemdaten', 'kps')) . '
**************************
ID: ' . $id . '
'. esc_html(__('Erstellt am', 'kps')) . ': ' . $this->_setDateTime . '
'. esc_html(__('Freigegeben am', 'kps')) . ': ' . $this->_unlockDateTime . '
'. esc_html(__('Löschzeitpunkt', 'kps')) . ': ' . $this->_deleteDateTime . '

'. esc_html(__('Werbung/Spam', 'kps')) . ': ' . $this->_reportCount['spam'] . '
'. esc_html(__('Unangemessen/Gewalt', 'kps')) . ': ' . $this->_reportCount['unreasonable'] . '
'. esc_html(__('Doppelter Eintrag', 'kps')) . ': ' . $this->_reportCount['double'] . '
'. esc_html(__('Persönlichkeitsrechte', 'kps')) . ': ' . $this->_reportCount['privacy'] . '
'. esc_html(__('Sonstiges', 'kps')) . ': ' . $this->_reportCount['others'] . '

' . esc_html(__('Eintrag', 'kps')) . '
**************************
' . $this->_authorContent . '
...
' . get_option('kps_mailFrom', false);
        ...
        // Email versenden
        $this->_isSend = wp_mail( esc_attr($this->_emailCopyCC['kpsEmailReport']), $reportSubject, $reportContent);

        // Email versenden Auto-Sperre
        if ($this->_lockedAutoReport === false && $this->_sendReportAuto === true)
        {
            $this->_isSendReportAuto = wp_mail( esc_attr($this->_emailCopyCC['kpsEmailReport']), $reportSubject, $reportContent);

            if(($this->_emailCopyCC['kpsEmailReport'] != get_option('kps_mailFrom', false)) && $this->_sendReportAuto === true)
            {
                $this->_isSendReportAdmin = wp_mail( esc_attr(get_option('kps_mailFrom', false)), $reportSubject, $reportContent);
            }
        }
    ...
    public function get_report($dbReport = NULL, $newReport = NULL, $reportSettings = NULL)
    {
            ...
            switch ($newReport)
            {
                case '0':
                    ...
                    $reportReason               = esc_html(__('Werbung/Spam', 'kps'));
                ...
                case '1':
                    ...
                    $reportReason               = esc_html(__('Unangemessen/Gewalt', 'kps'));
                ...
                case '2':
                    ...
                    $reportReason               = esc_html(__('Doppelter Eintrag', 'kps'));
                ...
                case '3':
                    ...
                    $reportReason               = esc_html(__('Persönlichkeitsrechte', 'kps'));
                ...
                case '4':
                    ...
                    $reportReason               = esc_html(__('Sonstiges', 'kps'));
