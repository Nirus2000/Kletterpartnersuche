...
class kps_entry_write
{
    ...
    public function __construct($write = '', $pageUrl = '')
    {
        ...
        // Freigabezeitstempel
        $unlockDateTime = ($this->_userSettings['kpsUserRequireAdminUnlock'] === 'false') ? time() + get_option('kps_deleteNoEntryTime', false) : 0;
        ...
        if ($this->_isNotFound === true)
        {
                    ...
                    && $this->_authorEmailCheck !== false)
            {
                ...
                $this->_insertDB = true;
                $this->insert($hash, $deletePassword, $activationHash, $deleteHash, $userRequireAdminUnlock, $unlockDateTime);

                // Aktivierungs-Email versenden
                if ($this->_isInsertDB > 0)
                {
                    $this->sendActivation($deletePassword, $activationHash, $deleteHash, $unlockDateTime, $pageUrl);
                    ...
                    if ($this->_emailCopyCC['kpsEmailCCIsActivated'] === 'true')
                    {
                        $this->sendEmailCopy();
                    }
                }
     ...
     */
    public function insert($hash = '', $deletePassword = '', $activationHash = '', $deleteHash = '', $userRequireAdminUnlock = 0, $unlockDateTime = 0)
    {
        global $wpdb;
     ...
     * Aktivierungs-Email versenden
     */
    public function sendActivation($deletePassword = '', $activationHash = '', $deleteHash = '', $unlockDateTime = 0, $pageUrl= '')
    {
        ...
        {
            $authorMailSubject  = esc_html(__('Aktivierung', 'kps'));
            $auhorMailContent   =
...
%blogemail%', 'kps'));
        ...
        $unlockDateTime = ($unlockDateTime === 0) ? esc_html(__('Warte auf Freigabe vom Autor', 'kps')) : $unlockDateTime;
        ...
        // Email versenden
        $this->_activationEmailIsSend = wp_mail( esc_attr($this->_authorEmail), $authorMailSubject, $auhorMailContent );
    ...
    public function sendAdminActivation()
    {
        ...
        $adminActivationSubject = esc_html(__('Freigabe erforderlich', 'kps')) . ': ' . $this->_blog_name;
        $adminActivationMessage = esc_html(__('Ein neuer Eintrag in der Kletterpartner-Suche ist vorhanden und erfordert deine Freigabe!', 'kps'));
...
' . $this->_blog_url;
        ...
        $this->_adminActivationIsSend = wp_mail( esc_attr(get_option('kps_MailFrom', false)), $adminActivationSubject, $adminActivationMessage );
    ...
    {
        $emailCopySubject = esc_html(__('Neuer Eintrag', 'kps')) . ': ' . $this->_blog_name;
        $emailCopyMessage = esc_html(__('Es gibt einen neuen Eintrag in der Kletterpartner-Suche!', 'kps'));
        $emailCopyMessage .= '

' . $this->_blog_name . '
' . $this->_blog_url;

        $this->_emailCopyIsSend = wp_mail( esc_attr($this->_emailCopyCC['kpsEmailCC']), $emailCopySubject, $emailCopyMessage );
