...
function kps_PrivacyErasre($erasers) {
  $erasers['Kletterpartner-Suche'] =    array(
                                            'eraser_friendly_name' => esc_html(__( 'Kletterpartner-Suche' )),
...
function kps_PrivacyErasure($authorEmail) {
        ...
        && (!is_array($resultVerifications) OR empty($resultVerifications)))
    {
        ...
        'items_removed'  => true,
        'items_retained' => true,
        'messages'       => esc_html(__( 'Keine Einträge oder Anforderungen in der Kletterpartner-Suche gefunden!')),
        'done'           => true
        );
    }
    ...
    if (is_array($resultEntries) && count($resultEntries) > 0)
    {
        foreach ($resultEntries as $entries)
        {
            ...
            $messages[] = esc_html(__('Kletterpartner-Suche')) . ':&#160;' . esc_html(__('Eintrag gelöscht')) . '&#160;ID:&#160;' . $entries->id;
        }
    }
    else
    {
        $messages[] = esc_html(__('Kletterpartner-Suche')) . ':&#160;' . esc_html(__('Keine Einträge gefunden!'));
    }
    ...
    if (is_array($resultVerifications) && count($resultVerifications) > 0)
    {
        foreach ($resultVerifications as $verifications)
        {
            if ($verifications->sendData === '0')
            {
                ...
                $messages[] = esc_html(__('Kletterpartner-Suche')) . ':&#160;' . esc_html(__('Anforderung gelöscht')) . '&#160;ID:&#160;' . $verifications->id;
            }
            else
            {
                // Löschen nicht möglich
                $messages[] = esc_html(__('Kletterpartner-Suche')) . ':&#160;' . esc_html(__('Löschen nicht möglich! Sie habe Kontaktdaten eines anderen Autoren abgerufen.'));
            }
        }
    }
    else
    {
        $message[] = esc_html(__('Kletterpartner-Suche')) . ':&#160;' . esc_html(__('Keine Einträge in den Anforderungen gefunden!'));
    }
...
function register_kps_Data_exporter($exporters) {
    $exporters['Kletterpartner-Suche'] =  array(
                                        'exporter_friendly_name'    => esc_html(__('Kletterpartner-Suche')),
...
function kps_PrivacyExporter($authorEmail) {
    ...
    $entryInformationsToExport = array( 'authorId'          => esc_html(__('User-Id', 'kps')),
                                        ...
                                        'formOptions'       => esc_html(__('zusätzliche Kontaktmöglichkeiten', 'kps')),
                                        'authorIp'          => esc_html(__('IP-Adresse', 'kps')),
                                        'authorHost'        => esc_html(__('Host', 'kps')),
                                        'setDateTime'       => esc_html(__('Erstellt', 'kps')),
                                        'unlockDateTime'    => esc_html(__('Freigegeben', 'kps')),
                                        'deleteDateTime'    => esc_html(__('Löschzeitpunkt', 'kps')),
                                        'authorSearchfor'   => esc_html(__('Ich suche für', 'kps')),
                                        'authorRule'        => esc_html(__('Art der Suche', 'kps')),
                                        'yourRule'          => esc_html(__('Ich bin', 'kps')),
                                        'content'           => esc_html(__('Eintrag', 'kps')),
                                        'url'               => esc_html(__('URL', 'kps')),
                                        'verifications'     => esc_html(__('Kontaktdaten versendet an', 'kps'))
	);
    ...
    if (is_array($resultEntries) && count($resultEntries) > 0)
    {
        foreach ($resultEntries as $entries)
        {
                	...
                	case 'formOptions':
                        ...
                        if (!empty($setAuthorContactInfo) AND $setAuthorContactInfo != "" AND is_array($setAuthorContactInfo))
                        {
                            foreach ($setAuthorContactInfo AS $key => $value)
                            {
                                ...
                                {
                                    $setAuthorContactInfoData .= esc_html(__('Telefon', 'kps')) . ": " . $value . "<br />";
                                }
                                elseif( $key == 'authorMobile')
                                {
                                    $setAuthorContactInfoData .= esc_html(__('Handy', 'kps')) . ": " . $value . "<br />";
                                }
                                ...
                                {
                                    $setAuthorContactInfoData .= esc_html(__('Webseite', 'kps')) . ": " . $value . "<br />";
                                }
                    	...
                    	case 'url':
                            ...
                            {
                                $value = esc_html(__('Diese Eintrag ist nicht sichtbar!', 'kps'));
                            }
                   		break;
                    	case 'verifications':
                            ...
                            {
                                $value = esc_html(__('Keine personenbezogenen Daten versendet!', 'kps'));
                            }
                   		break;
                    }
            ...
            $dataToExport[] = array(
            	'group_id'    => 'kps',
            	'group_label' => esc_html(__( 'Kletterpartner-Suche Einträge', 'kps' )),
