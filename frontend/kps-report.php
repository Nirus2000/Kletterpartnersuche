...
function kps_report($reportId = '0')
{
        ...
        && $captchaValid === true)
    {
        ...
        if ($report->show_isReport())
        {
            // Messagebox Ausgabe
            $messageboxContent = '
            <div style="text-align: center;">' . esc_html(__('Wir werden schnellstmöglich deine Meldung bearbeiten!', 'kps')) . '</div>
            ';
            $output = kps_messagebox(esc_html(__('Eintrag gemeldet', 'kps')) , $messageboxContent);
        }
        ...
        else
        {
            ...
            $output = kps_messagebox(esc_html(__('Fehler! Eintrag konnte nicht gemeldet werden!', 'kps')) , $messageboxContent);
        }
    }
            ...
            && isset($_POST['submit']))
    {
            ...
            $messageboxContent = '
            <ul>
                <li>' . esc_html(__('Es wurde kein Meldungsgrund ausgewählt!', 'kps')) . '</li>
                <li>' . esc_html(__('Die Eintrag-ID ist falsch!', 'kps')) . '</li>
                ' . $captchaFail . '
