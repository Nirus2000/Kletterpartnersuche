...
class kps_entry_read
{
    ...
    public      $_authorId,                 // Autor Id
                ...
                $_allowedAvatar,            // Autor-Avatar erlaubt
                $_allowedReport,            // Reorting erlauben
                $_userProfilLink,           // Autor-Link
    ...
    public function __construct($id = 0)
    {
        ...
        $this->_allowedAvatar           = kps_unserialize( get_option( 'kps_userSettings', false ) )['kpsUserAvatar'];
        $this->_userProfilLink          = kps_unserialize( get_option( 'kps_userSettings', false ) )['kpsUserProfilLink'];
        $this->_allowedReport           = kps_unserialize( get_option( 'kps_userSettings', false ) )['kpsUserReport'];
        $this->_isFound                 = false;
        ...
        else
        {
            ...
            // Report erlauben
            $this->get_allowedReport($this->_allowedReport);
    ...
    public function get_authorData($authorId, $authorName, $authorEmail)
    {
        ...
        if (is_numeric($authorId) && $authorId > 0 && $authorData !== false)
        {
            ...
            if (isset($authorData->display_name) && !empty($authorData->display_name))
            {
                ...
                elseif ($this->_userProfilLink === 'false' && current_user_can('moderate_comments'))
                {
            ...
            else
            {
               ...
               if ($this->_userProfilLink === 'true')
                ...
                elseif ($this->_userProfilLink === 'false' && (current_user_can('editor') || current_user_can('administrator')))
                {
                    $this->_authorName = '<a href="' . esc_url(get_edit_user_link($authorData->id)) . '" title="' . esc_html(get_the_author_meta('user_login', $authorData->id)) . '">' . esc_html(get_the_author_meta('user_login', $authorData->id)) . '</a>';
                }
                else
        ...
        // Avatar erlaubt?
        if ($this->_allowedAvatar === 'true' && get_option('show_avatars') === '1')
        {
            // Avatar holen
    ...
    public function get_authorRule($authorRule = NULL, $iconPak = 0, $widgetIconPak = '25')
    {
        if (is_numeric($authorRule))
        {
            ...
            {
                case '0':
                    $alt    = esc_html(__('Einmalig', 'kps'));
                    $pic    = "onetime_" . $iconPak . ".png";
                    $widget = "onetime_" . $widgetIconPak . ".png";
                break;
                case '1':
                    $alt    = esc_html(__('Regelmäßig', 'kps'));
